...
export default function AuthForm({ mode }: { mode: 'sign-in' | 'sign-up' }) {
  ...
  const input = 'w-full rounded-xl border border-white/10 bg-[#111a2b] px-11 py-3.5 text-[15px] text-white outline-none transition placeholder:text-slate-600 focus:border-primary focus:ring-4 focus:ring-primary/10';
  return <form action={formAction} className="space-y-5" aria-describedby="auth-status">
    {mode === 'sign-up' && <>
      <div className="grid grid-cols-2 rounded-xl bg-black/20 p-1" role="tablist" aria-label="Registration step">
        <button type="button" role="tab" aria-selected={signupStep === 'request'} onClick={() => setSignupStep('request')} className={`rounded-lg px-3 py-2.5 text-sm font-bold transition ${signupStep === 'request' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-on-surface-variant'}`}>1. Request access</button>
        <button type="button" role="tab" aria-selected={signupStep === 'complete'} onClick={() => setSignupStep('complete')} className={`rounded-lg px-3 py-2.5 text-sm font-bold transition ${signupStep === 'complete' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-on-surface-variant'}`}>2. Complete signup</button>
      </div>
    ...
    <button disabled={pending} className="w-full rounded-xl bg-gradient-to-r from-primary to-primary-container px-5 py-3.5 font-bold text-white shadow-lg shadow-primary/20 transition disabled:cursor-not-allowed disabled:opacity-60">{pending ? 'Please wait…' : mode === 'sign-in' ? 'Sign in securely' : signupStep === 'request' ? 'Send approval request' : 'Create administrator account'}</button>
    <p className="text-center text-sm text-on-surface-variant">{mode === 'sign-in' ? <>Need administrator access? <Link className="font-semibold text-primary" href="/admin/auth/sign-up">Request access</Link></> : <>Already registered? <Link className="font-semibold text-primary" href="/admin/auth/sign-in">Sign in</Link></>}</p>
  </form>;
}
